...
download.file('https://github.com/ICPSR/IDS/raw/master/User_Guide/UG_example_1.zip', destfile = 'UG_example_1.zip') 
unzip('UG_example_1.zip',list = T)  
unzip('UG_example_1.zip',files = 'UG_Example_1_v10.xlsx' )  
file.remove('UG_example_1.zip')
```
...
Las transformaciones que se aplicaran sobre las variables de los fichero fuentes se define en un libro excell con una estuctura especifica (Se incluyen un ejemplo de este libro en el directorio *inst/extdata/* del paquete) para opterne los registros que compone el IDS.  A continuacion se muestra un ejemplo de generacion, con los datos previamente descargados.
```{r}
##--------------------------------------------------------------------------
...
La ejecucion del codigo anterior produce un una lista con 5 elemntos, siendo cada uno de ellos una data.table que sigue las especificaciones del formato IDS.

Desde este objeto se pueden opten fichero en formato CSV, para ser icorporado, a una base de datos SQL. Las siguientes ordenes realizan estos pasos
```{r}
